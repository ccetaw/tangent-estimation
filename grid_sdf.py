import taichi as ti
from taichi.math import vec3, atan2, pi, sqrt, acos, normalize, mat4, sin, vec4, dot, length

@ti.func
def sd_sphere(o: vec3, r: float, p: vec3):
    """
...
class GridSDF:
    ...
    @ti.kernel
    def init_sphere_sd(self, o: vec3, r: float):
        """
        Initialize a sphere sdf.
        """
        for i, j, k in self.density:
            p = self.lb + vec3(i,j,k) * self.units
            self.density[i,j,k] = sd_bunny(o, r, p)

    @ti.kernel
    def init_bunny_sd(self):
        for i, j, k in self.density:
            p = self.lb + vec3(i,j,k) * self.units
            self.density[i,j,k] = sd_bunny(p)
